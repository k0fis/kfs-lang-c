...
#define CE(name, res) if (res) { KFS_ERROR("%s : %i", name, res); return res; }
...
int main(void) {
   ...
   CE("Array", eval(kfsLangEnv, "[[[[[]]]], []]", NULL, NULL, NULL, 0));
   CE("Array empty", eval(kfsLangEnv, "[]", NULL, NULL, NULL, 0));
   iExpected =  1; CE("De-Array", eval(kfsLangEnv, "[1]", NULL, &iExpected, NULL, 0));
   CE("Array", eval(kfsLangEnv, "[1, 2]", NULL, NULL, NULL, 0));
   iExpected = !0; CE("true", eval(kfsLangEnv, " true", NULL, &iExpected, NULL, 0));
   iExpected =  0; CE("false", eval(kfsLangEnv, " false", NULL, &iExpected, NULL, 0));
   iExpected = !0; CE("true", eval(kfsLangEnv, " (5 != 3) && (3 > (2-5)) ", NULL, &iExpected, NULL, 0));
   iExpected = !0; CE("true", eval(kfsLangEnv, "3 <= (2+1) ", NULL, &iExpected, NULL, 0));

   eval(kfsLangEnv, " { a : 1 ; b: 12.0/3.1; c : (12+4)}  ", NULL, NULL, NULL, 0);
   dExpected = 3.87;  CE("dbl", eval(kfsLangEnv, " { a : 1 ; b: 12.0/3.1; c : (12+4)}.b  ", NULL, NULL, &dExpected, 0.01));

   dExpected = 2; CE("dbl", eval(kfsLangEnv, " [1.0, 2.0][1] ", NULL, NULL, &dExpected, 0));
   iExpected = 0; CE("int()", eval(kfsLangEnv, "int( [1.0, {a : [2.0]}][1].a[0] -2) ", NULL, &iExpected, NULL, 0));
